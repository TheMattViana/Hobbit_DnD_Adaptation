// script.js for Hobbit Adventures Character Sheet (v7.1.3 - UI Toggles & Data Management Styling)

// --- CONSTANTS AND DATA ---
const SHEET_VERSION = "7.1.3"; // Updated version
...
function addItemToInventoryFromSelect() {
    ...
    if (itemData) { addItemToInventory(JSON.parse(JSON.stringify(itemData))); select.value = ""; }
...
function applyCharacterData(data) {
    ...
    if (data.currentCharacterSkillPoints === undefined || (data.version && parseFloat(data.version) < 7.1)) { 
     ...
     if (data.abilityPointsAvailable === undefined && (!data.version || parseFloat(data.version) < 7.1)) {
        ...
        characterState.abilityPointsAvailable = Math.max(0, INITIAL_ABILITY_SCORE_POINTS - pointsSpentFromOldStats);
    ...
    updateMaxFortitudeDisplay(); 
    if (data.fortitudeCurrent > getValue('fortitudeMax')) setValue('fortitudeCurrent', getValue('fortitudeMax'));
    else setValue('fortitudeCurrent', data.fortitudeCurrent || getValue('fortitudeMax'));
}
...
function loadCharacterData() { const savedDataJSON = localStorage.getItem(LOCAL_STORAGE_KEY); if (savedDataJSON) { try { const loadedData = JSON.parse(savedDataJSON); if (loadedData.version !== SHEET_VERSION && parseFloat(loadedData.version || "0") < parseFloat(SHEET_VERSION)) { if (!confirm(`Loading data from an older version (Saved: ${loadedData.version || 'pre-7.1'}, Current: ${SHEET_VERSION}). Some features or data might need manual review/adjustment. Continue?`)) { logDiceRoll('System', 'Load cancelled: version mismatch.', ''); return false; } } applyCharacterData(loadedData); logDiceRoll('System', 'Character Data Loaded!', `Version: ${loadedData.version || 'unknown'}`); return true; } catch (e) { console.error("Error parsing saved data:", e); alert("Error loading character data. It might be corrupted. Starting with a new sheet."); logDiceRoll('System', 'Error loading data.', 'Starting fresh.'); initializeNewSheet(); return false; } } return false; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!loadCharacterData()) {
        ...
        applyUIVisibility(); // Ensure UI visibility is applied after loading
    }
